  ...
  const handleSubmit = () => {
    ...
    create(
      ...
      },
      {
        ...
        onError: (e: any) =>
          setError(
            e?.response?.data?.message ?? "Failed to assign subscription.",
          ),
      },
    );
  };
  ...
  return (
    <ModalShell title="Assign Subscription" onClose={onClose}>
      ...
      <ModalFooter
        ...
        isPending={isPending}
        confirmLabel="Assign Subscription"
...
export function DeleteSubDialog({
  sub,
  onClose,
}: {
    ...
    <ModalShell title="Delete Subscription" onClose={onClose}>
      <p className={styles.deleteBody}>
        ...
        <strong className={styles.deleteHighlight}>
          {getMember(sub)?.name ?? "this member"}
        </strong>
        ? This action cannot be undone.
      </p>
      <ModalFooter
        onClose={onClose}
        onConfirm={() => del(sub._id, { onSuccess: onClose })}
        isPending={isPending}
