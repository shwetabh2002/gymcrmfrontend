      ...
      >
        ...
        <span className={styles.comboboxSuffix}>
          {selected ? (
            ...
            <span
              className={`${styles.comboboxChevron} ${open ? styles.comboboxChevronOpen : ""}`}
            >
              ▾
            </span>
          )}
  ...
  return (
    <ModalShell title="Record Payment" onClose={onClose}>
      ...
      <div className={styles.formGrid}>
        <Field
          label={`Member * (${activeMembers.length} with active subscription)`}
        >
          <MemberCombobox
            members={activeMembers}
            value={form.memberId ?? ""}
            onChange={(id) => {
              setForm((p) => ({ ...p, memberId: id, subscriptionId: "" }));
              setError("");
            }}
          />
        </Field>
        ...
        <Field label="Subscription *">
          <select
            ...
            onChange={(e) => {
              setForm((p) => ({ ...p, subscriptionId: e.target.value }));
              setError("");
            }}
        ...
        <div className={styles.twoCol}>
          <Field
            label={
              selectedSub
                ? `Amount ₹ (max ₹${selectedSub.pendingAmount.toLocaleString()})`
                : "Amount (₹) *"
            }
          >
            <input
        ...
        <Field
          label={`Payment screenshot (optional) · max ${uploadLimits.maxFileMb}MB`}
        >
          {proofPreview ? (
          ...
          <div className={styles.paymentSummary}>
            <div className={styles.paymentSummaryDetail}>
              ...
              <span className={styles.textSuccess}>
                ₹{selectedSub.totalPaid.toLocaleString()}
              </span>
              {" · "}Pending:{" "}
              <span className={styles.textWarning}>
                ₹{selectedSub.pendingAmount.toLocaleString()}
              </span>
            </div>
          </div>
        )}
